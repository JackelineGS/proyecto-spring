...
public class PanelCitaService {
    ...
    public List<String> diasSemana() {
        return List.of("Lunes", "Martes", "Miércoles", "Jueves", "Viernes");
    }

    public List<CitaPanelDto> listarCitas(Long empleadoId) {
        LocalDate hoy = LocalDate.now();
        LocalDate lunes = hoy.with(DayOfWeek.MONDAY);
        LocalDate viernes = lunes.plusDays(4);
        return citaRepository.findByEmpleadoAndRangoFechas(empleadoId, lunes, viernes).stream()
                .map(this::toDto)
                .toList();
    }

    public List<List<CitaPanelDto>> citasPorDia(Long empleadoId) {
        List<CitaPanelDto> citas = listarCitas(empleadoId);
