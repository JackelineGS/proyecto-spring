(function () {
    ...
    function actualizarTextosReserva() {
        ...
        var esp = reservaForm.dataset.empleadoNombre || '';
    ...
    function validateStep(step) {
        if (step === 0) {
            var esp = reservaForm.querySelector('[name="empleadoId"]');
        ...
        if (step === 2) {
            var required = ['nombre', 'apellido', 'fechaNacimiento', 'celular', 'correo', 'dni'];
    ...
    document.querySelectorAll('[data-select-empleado]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            var id = btn.getAttribute('data-id');
            var nombre = btn.getAttribute('data-nombre');
            reservaForm.querySelector('[name="empleadoId"]').value = id;
            reservaForm.dataset.empleadoNombre = nombre;
            var label = document.getElementById('empleado-seleccionado');
    ...
    var horariosPorDiaCache = {};

    function cargarHorarios(empleadoId) {
        var grid = document.getElementById('horarios-grid');
        if (!grid) return;
        grid.innerHTML = '<p class="text-muted">Cargando horarios...</p>';
        fetch('/api/horarios/' + empleadoId)
            .then(function (r) { return r.json(); })
            .then(function (mapa) {
                horariosPorDiaCache = mapa;
                renderizarHorasDeFecha(calSelected);
            })
            .catch(function () {
    ...
    function renderizarHorasDeFecha(dateObj) {
        var grid = document.getElementById('horarios-grid');
        if (!grid) return;
        var horaInput = reservaForm.querySelector('[name="hora"]');
        if (horaInput) horaInput.value = '';
        actualizarTextosReserva();

        if (!dateObj) {
            grid.innerHTML = '<p class="text-muted">Selecciona una fecha en el calendario.</p>';
            return;
        }

        var nombresDias = ["Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado"];
        var diaSemana = dateObj.getDay(); // 0=Dom, 1=Lun...
        var nombreDia = nombresDias[diaSemana];
        var horas = horariosPorDiaCache[nombreDia] || [];

        grid.innerHTML = '';
        if (!horas.length) {
            grid.innerHTML = '<p class="text-muted">No hay horarios disponibles este día.</p>';
            return;
        }
