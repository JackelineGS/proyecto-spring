(function () {
    ...
    function renderEscalas() {
        var grid = document.getElementById('hc-escalas-grid');
        if (!grid) return;
        grid.innerHTML = '';
        hcData.escalas.forEach(function (e, i) {
            var card = document.createElement('div');
            card.className = 'card panel-card';
            card.innerHTML =
                '<div style="display:flex;justify-content:space-between;gap:0.5rem;align-items:flex-start;">' +
                '<p style="margin:0;flex:1;"><strong>' + e.nombre + '</strong></p>' +
                '<button type="button" class="btn btn-outline btn-sm btn-edit-escala" data-index="' + i + '">✎ Editar</button>' +
                '</div>' +
                '<p class="text-muted">' + e.interpretacion + '</p>' +
                '<p>Puntaje: <strong>' + e.puntaje + '</strong></p>';
            grid.appendChild(card);
        });
        grid.querySelectorAll('.btn-edit-escala').forEach(function (btn) {
            btn.addEventListener('click', function () {
                abrirModalEscala(parseInt(btn.getAttribute('data-index'), 10));
            });
        });
    }
    ...
    function abrirModalEscala(index) {
        escalaEditIndex = index != null ? index : null;
        var e = index != null ? hcData.escalas[index] : null;
        document.getElementById('modal-escala-titulo').textContent = e ? 'Editar escala' : 'Nueva escala';
        document.getElementById('escala-nombre').value = e ? e.nombre : '';
        document.getElementById('escala-interp').value = e ? e.interpretacion : '';
        document.getElementById('escala-puntaje').value = e ? e.puntaje : '';
        openModal('modal-escala');
    }

    document.getElementById('btn-add-escala') && document.getElementById('btn-add-escala').addEventListener('click', function () {
        abrirModalEscala(null);
    });

    document.getElementById('btn-guardar-escala') && document.getElementById('btn-guardar-escala').addEventListener('click', function () {
        var item = {
            nombre: document.getElementById('escala-nombre').value.trim(),
            interpretacion: document.getElementById('escala-interp').value.trim(),
            puntaje: document.getElementById('escala-puntaje').value.trim()
        };
        if (!item.nombre || !item.interpretacion || !item.puntaje) return;
        if (escalaEditIndex !== null) {
            hcData.escalas[escalaEditIndex] = item;
        } else {
            hcData.escalas.unshift(item);
        }
        document.getElementById('modal-escala').classList.remove('open');
        renderEscalas();
    });
